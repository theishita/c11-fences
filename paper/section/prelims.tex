...
$\tau$ with ordering restriction \rel. \svs{Why not use $o \in \moset$ representing order as a replacement?}
...
\noindent
{\bf Buggy and fixed executions}: A \deleted{ threads of} 
program $P$ may contain {\em assert} \added{instructions}  \deleted{as a means of providing} 
\added{ as correctness}
\deleted{program} specification. \added{$P$ is considered buggy} when a trace of $P$ 
...
\added {Let the} 
buggy trace \added{be denoted by} $\tau$, \deleted{by} \added{and the
corresponding fixed trace is denoted by} $\fx{\tau}$. 
%
