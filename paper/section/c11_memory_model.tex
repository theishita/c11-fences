The \cc memory model \added{introduces the {\em happens-before} relation},
which is an irreflexive and acyclic relation over the events of 
an \deleted{execution sequence} \svs{again: should it be trace?}
$\tau$ called  
($\setHB \subseteq \events_\tau {\times} \events_\tau$);
such that, \svs{the execution sequence(?)} is {\em coherent} (or valid) under \cc if 
...
\begin{itemize}
	...
	% (where, a release sequence headed by a $\moge$\rel write
	...
	$\moge$\rel write events from other threads.) \comment{Quite
	unparsable! I should get the jist in just 2-3 sentences, which
	...
	write when the read reads-from the write. \comment{may be better to show via diagram
        and explain without using too many symbols.}
...
\begin{definition}[\sc{\em -from-read} $\setFR$]\newline
	...
	if $(r^\sc,w^\sc)$ $\in$ $\setRF^{-1};\setMO$ then $\fr{\tau}{r^\sc}{w^\sc}$.
        \svs{What do you mean here by $R1;R2$? Also, what is $\setRF^{-1}$; As a suggestion
        -- it is better to use names for relations than math symbols.}
\end{definition}
