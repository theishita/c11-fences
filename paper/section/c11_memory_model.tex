...
%
%
Let $\setRF$ represent a {\em reads-from} relation \st 
$\rf{\tau}{w_1}{r_1}$ signifies that $r_1 \in \reads_\tau$ 
read its value from $w_1 \in \writes_\tau$ in trace $\tau$.
The events of a trace $\tau$ are related by the $\setHB$ as 
follows;
\begin{itemize}
	\item {\em Intra-thread-hb}: Events of a thread $P_i$  are 
		related by {\em sequenced-before} relation ($\setSB$) in 
	...
	\item {\em Inter-thread-hb}: When a strict write $e_w$ (memory order
		...
		$\seqb{\tau}{e_r}{e'}$ are also related as $\ithb{\tau}{e}{e'}$.
                \svs{If there are other rules that introduce ithb edges among events,
                then mention it here and say that why they are not need for the
                exposition of this work.}
\end{itemize}

Accordingly, two events $e_1,e_2$ in a trace $\tau$ are happens-before 
related \ie
$\hb{\tau}{e_1}{e_2}$ if $\seqb{\tau}{e_1}{e_2}$ $\v$ $\ithb{\tau}{e_1}{e_2}$.
\svs{Is $\hb{\tau}{}{}$ = $\seqb{\tau}{}{} \cup \ithb{\tau}{}{}$ or
$\hb{\tau}{}{} \Rightarrow \seqb{\tau}{}{} \cup \ithb{\tau}{}{}$?}
%
Further, in a trace $\tau$, all write events of an object $x$ are related by a 
total order called {\em modification-order} ($\setMO$).
%
The $\setMO$ order is constructed in compliance to $\setHB$ and $\setRF$. 
%
...
reads with writes ordered {\em after} it. \svs{Why is there a need for this
relation? Please motivate!}
...
\end{definition}
% 
A coherent \cc trace must:
(i) forbid read of a write that is happens-before 
hidden by a later write in $\setMO$, in other words, 
$\neg(\hb{\tau}{e_1}{e_2}$ $\^$ $\fr{\tau}{e_2}{e_1})$;
%
(ii) form a total order on all events with memory
order \sc, called \sc{\em -total-order} ($\setTO$) 
\cite{LahavVafeiadis-PLDI17} \st
$\forall e^\sc_1, e^\sc_2 \in \ordevents{\sc}$,

\svs{The rules below start abruptly. Please stitch up some text before
you list the rules. Why are the needed? What are you going to do with them? }
...
\svs{Give couple of pictorial examples and use them in your explanations.}
