...
at appropriate locations that would force a cyclic $\setSO$ order in the \sc 
ordered events of a buggy trace $\tau$. The cycle invalidates the trace 
...
%$\setFR{\implies}\setSO$) 
 construct the $\setSO$ edges between the program events and synthesized 
 fences to form a cyclic $\setSO$ relation shown in \hlref{mutex}(ii).
...
form $\setSW$ and $\setDOB$ relations (formally described
as base $\setSW$ and $\setDOB$ rules),
where $\setSW \subseteq \setITHB$ and $\setDOB \subseteq \setITHB$.
%
The $\setSW$ and $\setDOB$ relations can similarly be formed between
fences based on appropriate $\setRF$, formally described as
extended $\setSW$ and $\setDOB$. The rules have been formally presented 
...
making previously available writes, unavailable to read events.
%
...
The fences $\sw{\inv{\tau}}{\mathbb{F}^\rel_1}{\mathbb{F}^\acq_2}$ form
...
recognize the program locations where a synchronization was 
absent. To do so we introduce $\lws$-rules 
(Fig~\ref{fig:so rules}(iii)) that essentially
signify the absence of a synchronization.
