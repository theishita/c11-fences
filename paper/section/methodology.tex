...
%We create a Boolean formula for every \lso-cycle every counter-example. 
...
Hence we need to insert $ (\bigvee\limits_{c \in \cycles{\im{\tau}}} (\bigwedge\limits_{\fn{} \in \ordevents{\sc}_c \intersection 
\ordfences{\sc}} \fn{})) $ to stop a trace $ \tau $. 
We use $\formula{\tau}$ to represent the boolean formula 
$ (\bigvee\limits_{c \in \cycles{\im{\tau}}} 
(\bigwedge\limits_{\fn{} \in \ordevents{\sc}_c \intersection 
\ordfences{\sc}} \fn{})) $. 
Clearly, any satisfying assignment to $ \formula{\tau} $ will give list of fences required to stop the buggy execution $ \tau $.
