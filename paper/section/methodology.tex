...
\begin{algorithm}
	...
	\begin{algorithmic}[1]	
		...
		\For{$ \tau \in \mathcal{CE} $}
			...
			\State $ \seqb{\tau'}{}{} := $ computeSB($\events_\tau^f$) \State $ \setSO := $ computeSO($\events_\tau^f, \setHB, \setMO, \setRF, \seqb{\tau'}{}{}$)
			\State cycles := computeCycles($ \setSO $)
			\If {cycles == $ \emptyset $}
...
Line 7 computes $ \setSO $-order in counter example $ \tau $ with candidate fences.
Line 8 finds all $ \setSO $-cycles. If there are no $ \setSO $-cycles 
present, the behavior can't be stopped used any \cc fences. Hence, we 
terminate the algorithm at this point.
Else, in line 12, we generate a SAT formula for the set of $ \setSO $-cycles and add it to formula $ \phi $
...
Each of these counter-examples $ \tau $ should give us a list program events 
$ \events_\tau $ and $ \setSB, \setHB, \setMO, \setSW $ relations over the 
program events. 
A program event consists of instruction label, action, memory order, and variable. 
