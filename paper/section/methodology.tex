...
%% computing relations in counter example
%% inserting candidate fences and computing SC
%%% mention a trivial solution here
%% Fence minimization

\divComment{Add details about \cc fences in \cc section.}
...
\ourtechnique takes a set of counter examples as input and 
computes \cc axiomatic relations, such as 
$ \setSB, \setHB, \setMO, \setSW $, 
among the events of counter examples.
Further we compute $ \setSO $-order assuming the existence of candidate fences.
...
cycle formed by transitively sequence of ordered pair in $ \setSO $-order. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\divComment{Replace it with counter example generator}
...
\divComment{Mention what is needed in the counter example}
