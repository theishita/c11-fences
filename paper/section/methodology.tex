...
% Why TO?
Our technique computes \setSC of buggy execution when pseudo fences are 
present. If such \setSC has a cycle, then the program with these fences 
...
% Other relations
%In order to find \setSC, we first compute the \setSB, \setSW, \setHB and 
%\setMO relations.
...
the trace. In particular, we are concerned with the \setSC (total order) 
...
% --------------- TO -----------
\subsubsection{Finding \setSC relations between SC instructions}\label{to_rel}
The final step in finding relations and the main step that is required for 
fence elimination is to find \setSC relations. The rules for \setSC relations 
...
Intuitively, transitive closure of \setSC is \emph{sc} ordering over the 
...
\begin{theorem}
	For any valid execution of a program, the transitive closure of \setSC 
...
The rules in \textsection\ref{to_rel} provides us \setSC relations between all 
instructions and fences. We now get a graph that would look 
like Fig \ref{fig:cycles}. 

%Further, the transitivity of \setSC 
%relation will give us some additional pair in \setSC. 
...
\setSC rules, if both of these relations hold true, then A = B. 
If A $\neq$ B and A and B are different entities which satisfy the 
relation ($\to{\tau}{A}{B}$ $\land$ $\to{\tau}{B}{A}$), that means that a cycle is 
formed between instructions A and B. This, in turn, violates the \setSC 
...
Note that apart from the rules described above, some \setSC edges 
will be created due to the transitivity of \setSC.
...
or not, and only those edges formed via the \setSC rules discussed in 
...
\par
We find out \setSC relations in only the buggy or erroneous traces which 
are obtain from verification tool. For each erroneous trace, a graph is 
formed with the instructions/fences as nodes and the \setSC relations as 
edges. 
...
that the buggy execution in which the \setSC cycle is introduces using 
...
\begin{lemma}
	Introducing fences in any cycle of \setSC will stop that cycle 
	from forming in actual run of the program.
\end{lemma}

\begin{lemma}
	If there are no cycles in \setSC of a buggy execution, the behavior 
	cannot be stopped using any C11 fences.
\end{lemma}

\begin{theorem}
	Stopping all cycles of \setSC in any buggy execution will stop that
...
and directly formed \setSC edges but not transitively formed \setSC edges.  
This is because transitively formed \setSC relations will be of 
no use in our case. If there is a transitive \setSC relation formed 
...
actual program that we did not include in \setSC, or ii) some 
...
transitive edge is formed due to some edge in our \setSC, there 
will be a representative cycle $C'$ of the cycle $C$ formed using 
edges in \setSC. As our formula only concerns the pseudo fences, 
...
\par
So even if transitive \setSC relations are considered as graph edges, 
