...
\divComment{TODO: add figures of counter examples in running example.}
...
\begin{figure}[!htb]
	\begin{center}
		\begin{tabular}{l||l}
			$ \store{y}{1}{\rlx} $ \quad \quad \quad \quad& $ \store{x}{1}{\rlx} $ \\
			$ \load{a}{x}{\rlx} $ & $ \load{c}{y}{\rlx} $ \\
			\textbf{if} $ a $ \textbf{then:} & \textbf{if} $ c $ \textbf{then:} \\
			\quad $ \store{z}{1}{\rlx} $ & \quad $ \store{z}{0}{\rlx} $ \\
			\quad $ \load{b}{z}{\rlx} $  & \quad$ \load{d}{z}{\rlx} $ \\
%			\quad assert($ z==1 $) & \quad assert($ z==0 $) \\
		\end{tabular}
