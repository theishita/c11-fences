...
{
	store_32(&var, 1);
	atomic_store_explicit(__LINE__, &x, *((int *)obj), memory_order_release);
	atomic_store_explicit(__LINE__, &x, *((int *)obj) + 1, memory_order_relaxed);
}

static void b2(void *obj)
{
	int r = atomic_load_explicit(__LINE__, &x, memory_order_acquire);
	printf("r = %d\n", r);
	store_32(&var, 3);
}
...
{
	atomic_store_explicit(__LINE__, &x, 22, memory_order_relaxed);
}
